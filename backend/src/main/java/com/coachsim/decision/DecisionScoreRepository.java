...
public interface DecisionScoreRepository extends JpaRepository<DecisionScore, Long> {
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM DecisionScore s WHERE s.fanDecisionId IN :fanDecisionIds")
    int deleteByFanDecisionIdIn(@Param("fanDecisionIds") Collection<Long> fanDecisionIds);
